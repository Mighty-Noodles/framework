...
import { sendSignupConfirmationRequiredEmail } from './SignupConfirmationRequired.email';
...
describe('PasswordReset Email', () => {
  ...
  describe('success', () => {
    test('sends email with password reset request link', async() => {
      ...
      expect(joinedEmail).toMatch(`https://${process.env.DOMAIN}/api/v1/signup/${user.id}/confirm?token=`);
